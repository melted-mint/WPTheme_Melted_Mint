            ...
            <li class="p-2 rounded-lg shadow-md grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-4 cardComponent">
                ...
                <div class="pl-2 order-2 sm:order-1">
                    ...
                    <div class="ml-2 text-md sm:text-lg">
                        <p>
                            <?php
                            // 'description' 메타데이터 가져오기 (선택적으로 사용)
                            $description = get_post_meta( get_the_ID(), 'description', true );
                            if ( ! empty( $description ) ) {
                                echo esc_html( $description );
                            } else {
                                // description이 없으면 비워둠
                                echo '';
                            }
                            ?>
                        </p>
                    ...
                    <!-- 날짜/수정일/카테고리/태그 -->
                    <div class="flex flex-row flex-wrap gap-x-4 gap-y-1 mt-1 sm:mt-2">

                        <!-- 날짜 및 수정일 -->
                        <div class="flex items-center text-xs sm:text-sm grayTextThings">
                        ...
                        <div class="flex items-center text-xs sm:text-sm grayTextThings">
                            ...
                            <div class="btn btn-ghost text-xs sm:text-sm rounded-lg h-7 sm:h-8 w-fit px-1 hoveronlyButton">
                                <?php the_category(''); // CPT가 'category'를 사용 중이어야 출력됨 ?>
                            </div>
                        </div><!-- 카테고리 -->
                    ...
                    $word_count        = str_word_count($content_stripped); // 영문 기준
...
<?php else: ?>
    <!-- 포스트가 없을 때 -->
    <p class="p-4 text-center">등록된 글이 없습니다.</p>
<?php endif; ?>