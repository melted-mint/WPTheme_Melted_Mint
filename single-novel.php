    ...
    <?php if ( have_posts() ) : ?>
        <?php while ( have_posts() ) : the_post(); ?>
            ...
            <article class="p-4 rounded-lg shadow-md cardComponent mb-6">
                ...
                <?php
                $one_liner = get_post_meta( get_the_ID(), 'one_liner', true );
                ?>
                <div class="p-4 border border-gray-300 rounded mb-6">
                    <strong>작가의 한마디 :</strong>
                    <div class="mt-1">
                        <?php 
                        // 한마디글 값이 있으면 출력, 없으면 그대로 비워둠
                        if ( ! empty( $one_liner ) ) {
                            echo nl2br( esc_html( $one_liner ) );
                        }
                        ?>
                    </div>
                </div>
...
<?php
get_template_part('templates/novel/footer-navigation');
