<?php
// 상위 '커뮤니티' 카테고리 객체와 ID를 가져옴
$community_cat = get_category_by_slug('커뮤니티');
$community_cat_id = $community_cat ? $community_cat->term_id : 0;

// '커뮤니티' 하위의 모든 카테고리 (hide_empty는 false로 처리 후 개별 체크)
$community_categories = get_categories(array(
    'child_of'   => $community_cat_id,
    'hide_empty' => false,
    'orderby'    => 'name',
    'order'      => 'ASC'
));
...
/**
 * 해당 카테고리에 'community' 포스트 타입의 글이 1개 이상 있는지 확인
 */
function has_community_posts_in_cat($cat_id) {
...
<div class="flex flex-col w-full">
    ...
    <div class="flex w-full">
        <ul class="w-full">
            <?php foreach ($community_categories as $category): ?>
                <?php
                // community 포스트 타입 글이 없는 카테고리는 출력하지 않음
                if ( ! has_community_posts_in_cat($category->term_id) ) {
                    continue;
                }
                ?>
