<?php
// 상위 '블로그' 카테고리 객체와 ID를 가져옴
$blog_cat = get_category_by_slug('블로그');
$blog_cat_id = $blog_cat ? $blog_cat->term_id : 0;

// '블로그' 하위의 모든 카테고리 (hide_empty는 false로 처리 후 개별 체크)
...
/**
 * 해당 카테고리에 'blog' 포스트 타입의 글이 1개 이상 있는지 확인
 */
function has_blog_posts_in_cat($cat_id) {
    ...
    <div class="flex w-full">
        <ul class="w-full">
            <?php foreach ($blog_categories as $category): ?>
                <?php
                // blog 포스트 타입 글이 없는 카테고리는 출력하지 않음
                if ( ! has_blog_posts_in_cat($category->term_id) ) {
                    continue;
                }
                ?>
                <li class="w-full hoveronlyButton rounded-lg h-10">
                    <a href="<?php echo esc_url(get_category_link($category->term_id).'blog'); ?>" 
