 ...
 * category.blog.php
 * 
 * 이 템플릿은 블로그 페이지 전용입니다.
 * URL 구조: /category/(카테고리슬러그)/blog
 * 예) /category/news/blog
 *      => category_name=news, mypage=blog (mypage는 고정되지 않음)
 */

/** ------------------------------
 * 2) 카테고리 파라미터 (페이지 선택은 고정)
 * ------------------------------ */
// rewrite 규칙을 통해 category_name이 넘어옴
$category_slug = get_query_var('category_name'); // 예: news, notice 등

// 블로그 페이지임이 확정되었으므로 매핑 로직 대신 고정값 사용
$current_post_type = 'blog';
$page_label = '블로그';

/** ------------------------------
 * 3) 메인 쿼리: 현재 CPT(블로그) + 카테고리
 * ------------------------------ */
...
$args = array(
    ...
    'orderby'        => 'date',
    'order'          => 'DESC',
    'category_name'  => $category_slug, // 카테고리 필터
);
$query = new WP_Query($args);

/** ------------------------------
 * 4) 사이드바용 카테고리 목록
 *    - 소설(novel), 외전(spinoff)에 쓰인 카테고리는 제외하고,
 *      블로그(CPT)에 글이 없는 카테고리도 제외
 * ------------------------------ */
...
/**
 * 새로운 함수 is_not_category_from_blog()
 * 블로그 전용에서 제외할 카테고리 판단:
 * - 소설 또는 외전에 사용된 카테고리
 * - 블로그 포스트가 한 개도 없는 카테고리
 */
function is_not_category_from_blog($cat_id) {
    // 소설/외전에 사용되었다면 제외
    ...
    $is_used_in_others = $check_query->have_posts();
    wp_reset_postdata();
    
    if ($is_used_in_others) {
        return true;
    }
    
    // 블로그 포스트가 한 개도 없다면 제외
    ...
    $has_blog_post = $blog_query->have_posts();
    wp_reset_postdata();
    
    return !$has_blog_post;
}
?>
    ...
    <aside class="order-last lg:order-none lg:sticky lg:top-12 w-full self-start">
        ...
        <div class="flex cardComponent rounded-xl px-2 py-2 max-w-[80rem] mx-auto sm:px-4 mb-4">
            ...
            <ul class="flex space-x-2">
                ...
                // 현재 URL의 mypage 값이 없더라도, 활성화 상태는 블로그로 고정
                $current_mypage = 'blog';
                foreach ($pages as $slug => $label):
                    $page_active_class = ($slug === $current_mypage) ? 'activatedButton' : '';
        ...
        <div class="w-full cardComponent p-4 rounded-xl">
            ...
            <ul class="space-y-1">
                <?php foreach ($all_categories as $cat): ?>
                    <?php
                    if ( is_not_category_from_blog($cat->term_id) ) {
                        continue;
                    }
                    $active_class = ($cat->slug === $category_slug) ? 'activatedButton' : '';
                    $cat_link = home_url('/category/' . $cat->slug . '/blog');
                    ?>
